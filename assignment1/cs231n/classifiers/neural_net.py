...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    scores -= np.array([np.max(scores, axis=1)]).T
    f_correct = scores[range(N), y]
    loss = np.mean(-f_correct + np.log(np.sum(np.exp(scores), axis=1)))
    ...
    p = np.exp(scores) / np.array([np.sum(np.exp(scores), axis=1)]).T
    p[range(N), y] -= 1
    dscores = p / N
