...
class CaptioningRNN(object):
    ...
    def sample(self, features, max_length=30):
        ...
        # forward rnn for each timestep
        prev_h = h0
        if self.cell_type == 'lstm': prev_c = np.zeros_like(h0)
        prev_caption = captions[:, 0].copy()
        for step in range(max_length):
            x, _ = word_embedding_forward(prev_caption, W_embed)
            ...
            idx_out = np.squeeze(np.argmax(scores, axis=2))
            captions[:, step] = idx_out

            # update for next loop
            prev_h = h
            if self.cell_type == 'lstm': prev_c = c
            prev_caption = captions[:, step].copy()
